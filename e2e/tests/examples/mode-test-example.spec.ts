...
testAllModes('can access main UI elements', async (page, mode) => {
  // At this point:
  // - Page is loaded
...
testModes(['local', 'gateway'], 'can create project without API key validation', async (page, mode) => {
  // This test only runs in Local and Gateway modes
  // (skipped in Remote mode which has stricter auth)

  const addProjectBtn = page.getByTitle('Add Project');
...
// Example 3: For simpler cases, you can also use regular Playwright tests
...
test('manual mode switching example', async ({ page }) => {
  await page.goto('/');
